...
export default function Home() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      // Sanity-check: Supabase URL must be real (not the placeholder)
    ...
    <div className="min-h-screen cohort-doodle-bg flex">
      ...
      <div style={{ flex: 1, marginLeft: '160px', display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#f6f7fa' }}>
        ...
        <main style={{ flex: 1, maxWidth: '640px', width: '100%', margin: '0 auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {activeTab === 'home' && (
            <>
              {/* Post Composer — only shown when logged in */}
              ...
              {!supabaseReady && !loadingPosts && (
                <div style={{
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '12.5px', color: '#78350f', lineHeight: 1.5 }}>
                      ...
                      <strong>NEXT_PUBLIC_SUPABASE_URL</strong> and <strong>NEXT_PUBLIC_SUPABASE_ANON_KEY</strong> from your{' '}
                      <a href="https://supabase.com/dashboard/project/_/settings/api" target="_blank" rel="noopener noreferrer" style={{ color: '#2563EB', textDecoration: 'underline' }}>
                        Supabase Dashboard
                      </a>. Then run the SQL in <code style={{ background: '#fef3c7', padding: '1px 5px', borderRadius: '4px', fontFamily: 'monospace' }}>supabase/schema.sql</code> and restart the dev server.
                    </p>
                  </div>
                </div>
              )}
                  ...
                  {[1, 2, 3].map(i => (
                    <div key={i} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                        <div style={{ width: 40, height: 40, borderRadius: '50%', backgroundColor: '#e2e8f0', animation: 'pulse 1.5s infinite' }} />
                        <div style={{ flex: 1 }}>
                          <div style={{ height: 12, width: '40%', backgroundColor: '#e2e8f0', borderRadius: 6, marginBottom: 6 }} />
                          <div style={{ height: 10, width: '25%', backgroundColor: '#f1f5f9', borderRadius: 6 }} />
                        </div>
                      </div>
                      <div style={{ height: 10, backgroundColor: '#f1f5f9', borderRadius: 6, marginBottom: 6 }} />
                      <div style={{ height: 10, width: '80%', backgroundColor: '#f1f5f9', borderRadius: 6 }} />
                    </div>
                  ))}
                </div>
              )}

              {/* Empty State — Supabase connected but no posts yet */}
              {!loadingPosts && supabaseReady && posts.length === 0 && (
                <div style={{
                  ...
                  <p style={{ fontSize: '13px', color: '#64748b', maxWidth: '280px', margin: '0 auto', lineHeight: 1.5, marginBottom: '16px' }}>
                    Be the first to post something on Cohort!{user ? '' : ' Sign in with Google to start posting.'}
                  </p>
                  {!user && (
                    <button
                      onClick={signInWithGoogle}
